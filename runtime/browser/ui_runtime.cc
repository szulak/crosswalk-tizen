...
bool HasAlwaysOnTopPermission(common::AppControl* appcontrol) {
  return strcmp(appcontrol->data("always_on_top").c_str(), "true") == 0;
}
...
void UiRuntime::OnAppControl(app_control_h app_control) {
  ...
  LOGGER(DEBUG) << "UiRuntime::OnAppControl(app_control_h app_control)";
  if (HasAlwaysOnTopCategory(
      app_data_->category_info_list()->categories)) {
    if (HasAlwaysOnTopPermission(appcontrol.get())) {
      if (native_window_->type() != NativeWindow::Type::NOTIFICATION)
        LOGGER(DEBUG) << "Creating notification window";
        ResetWebApplication(NativeWindow::Type::NOTIFICATION);
      } else {
        if (native_window_->type() == NativeWindow::Type::NOTIFICATION)
          ResetWebApplication(NativeWindow::Type::NORMAL);
      }
